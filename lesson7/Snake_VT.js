...
var FIELD_SIZE_X = 20;
var FIELD_SIZE_Y = 20;
var SNAKE_SPEED = 250;      // Интервал между перемещениями змейки
...
var food_deletion_timer     // Таймер исчезновения ячеек с едой
var problem_deletion_timer  // Таймер исчезновения проблемных ячеек
...
function startGame() {
    if (!gameIsRunning) {
        ...
        food_deletion_timer = setInterval(deleteFood, FOOD_SPEED);
        problem_deletion_timer = setInterval(deleteProblem, PROBLEM_SPEED);
    }
}
...
//Удаление ячеек с едой по таймеру

function deleteFood(){
    var foodDeleted = false;
    while(!foodDeleted){
        var food_del = document.getElementsByClassName('food-unit')[1];
        if(food_del !=undefined){
        var food_del_classes = food_del.getAttribute('class').split(' ');
        food_del_classes.splice(2,1);
            var classes = '';
            for (var i = 0; i < food_del_classes.length; i++) {
                classes += food_del_classes[i] + ' ';
            }
        food_del.setAttribute('class', classes);
        }
        foodDeleted = true;
        //console.log(classes);
    }
...
//Удаление проблемных ячеек по таймеру

function deleteProblem(){
    var problemDeleted = false;
    while(!problemDeleted){
        var problem_del = document.getElementsByClassName('problem-unit')[1];
        if(problem_del !=undefined){
        var problem_del_classes = problem_del.getAttribute('class').split(' ');
        problem_del_classes.splice(2,1);
            var classes = '';
            for (var i = 0; i < problem_del_classes.length; i++) {
                classes += problem_del_classes[i] + ' ';
            }
        problem_del.setAttribute('class', classes);
        }
        problemDeleted = true;
        //console.log(classes);
    }
